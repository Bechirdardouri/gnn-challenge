...
def main(
    ...
    train_per_class=20,
    val_per_class=60,
    label_noise=0.12,       # 12% flips in train only
    feature_dropout=0.30,   # 30% feature masking everywhere
):
    ...
    # ActorDataset note: edges are not provided both directions by default.
    ...
    # Dense low-dimensional features so CSVs stay small
    svd = TruncatedSVD(n_components=n_components, random_state=seed)
    Xd = svd.fit_transform(X)

    # Feature dropout everywhere (train/val/test)
